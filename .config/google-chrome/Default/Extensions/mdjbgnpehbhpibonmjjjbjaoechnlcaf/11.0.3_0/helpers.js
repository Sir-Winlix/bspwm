...
var api = {
	...
	getUrl: (endpoint, params) => {
		...
		let baseParams = {
			agent: env.config.client.agent,
			version: env.config.client.fullVersion,
		};

		params = Object.assign({}, baseParams, params); // Merge objects

		if(endpoint == 'link/redirector') {
			var url = env.config.apiSlowUrl + + 'v4/' + endpoint + '?' + uri.encodeQueryData(params); 
		}
		else if(endpoint == 'magnet/status') {
			var url = env.config.apiUrl + 'v4.1/' + endpoint + '?' + uri.encodeQueryData(params); 
		}
		else {
			var url = env.config.apiUrl + 'v4/' + endpoint + '?' + uri.encodeQueryData(params);
		}
	...
	fetch: async (endpoint, params, isRetry) => {
		...
		let url = await api.getUrl(endpoint, params);
